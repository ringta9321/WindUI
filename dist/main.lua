--[[
    ...
    Also call me Footages, Footagesus (Github)
    
    This script is not subject to change
...
function e.SetTheme(f)
e.Theme=f
end

local function ApplyCustomProps(f,g)
if g.ThemeTag then
e.AddThemeObject(f,g.ThemeTag)
end
end

function e.GetThemeProperty(f,g)
return g[f]
end

function e.AddThemeObject(f,g)
local h=#e.Objects+1
local i={
Object=f,
Properties=g,
...
function e.UpdateTheme()
for f,g in next,e.Objects do
for h,i in next,g.Properties do
e.Tween(f,0.08,{[h]=Color3.fromHex(e.GetThemeProperty(i,e.Theme))}):Play()
end
end
end

function e.Icon(f)
return d.assets["lucide-"..f]
end

function e.New(f,g,h)
local i=Instance.new(f)

for j,k in next,e.DefaultProperties[f]or{}do
i[j]=k
end

for l,m in next,g or{}do
if l~="ThemeTag"then
i[l]=m
end
end

for n,o in next,h or{}do
o.Parent=i
end

ApplyCustomProps(i,g)
return i
end

function e.Tween(f,g,h,...)
return c:Create(f,TweenInfo.new(g,...),h)
end
...
local n=m.Position-i
f.Position=UDim2.new(
l.X.Scale,l.X.Offset+n.X,
l.Y.Scale,l.Y.Offset+n.Y
)
end

f.InputBegan:Connect(function(m)
...
h.UIElements.SliderIcon=c("Frame",{
BackgroundTransparency=.9,
BackgroundColor3=Color3.fromHex(g.Theme.Text),
ThemeTag={
BackgroundColor3="Text",
...
m=game:GetService"UserInputService".InputEnded:Connect(function(r)
...
local l
l=b.InputBegan:Connect(function(m)
local n
...
b.InputBegan:Connect(function(l)
if l.KeyCode.Name==i.Value then
pcall(i.Callback,l.KeyCode.Name)
end
end)
return i.__type,i
end
...
local m={
__type="Dropdown",
Title=l.Title or"Dropdown",
Desc=l.Desc or nil,
Values=l.Values or{},
Value=l.Value or{},
AllowNone=l.AllowNone or true,
Multi=l.Multi,
...
m.UIElements.Dropdown=f("TextButton",{
BackgroundTransparency=.95,
Text="",
...
TextTransparency=.4,
TextXAlignment="Left",
BackgroundColor3=Color3.fromHex(l.Theme.Text),
Parent=m.DropdownFrame.UIElements.Main,
Size=UDim2.new(0,150,0,30),
AnchorPoint=Vector2.new(1,0.5),
TextTruncate="AtEnd",
Position=UDim2.new(1,0,0.5,0),
...
f("UIStroke",{
Color=Color3.fromHex(l.Theme.Text),
ThemeTag={
Color="Text",
},
Transparency=.93,
...
m.UIElements.UIListLayout=f("UIListLayout",{
Padding=UDim.new(0,h.MenuPadding/1.5),
FillDirection="Vertical"
})

m.UIElements.Menu=f("Frame",{
ThemeTag={
BackgroundColor3="Accent",
},
BackgroundTransparency=0.15,
...
m.UIElements.UIListLayout,
})
})
})

m.UIElements.MenuCanvas=f("Frame",{
Size=UDim2.new(0,170,0,300),
BackgroundTransparency=1,
Position=UDim2.new(-10,0,-10,0),
Visible=false,
Active=false,
Parent=l.Window.SuperParent.Parent.Dropdowns,
...
m.UIElements.Menu.CanvasGroup.ScrollingFrame.CanvasSize=UDim2.fromOffset(0,m.UIElements.UIListLayout.AbsoluteContentSize.Y)
end

local function RecalculateListSize()
if#m.Values>10 then
m.UIElements.MenuCanvas.Size=UDim2.fromOffset(m.UIElements.UIListLayout.AbsoluteContentSize.X,392)
else
m.UIElements.MenuCanvas.Size=UDim2.fromOffset(m.UIElements.UIListLayout.AbsoluteContentSize.X,m.UIElements.UIListLayout.AbsoluteContentSize.Y+10)
end
end

function UpdatePosition()
local n=0
if d.ViewportSize.Y-m.UIElements.Dropdown.AbsolutePosition.Y<m.UIElements.MenuCanvas.AbsoluteSize.Y+10 then
n=m.UIElements.MenuCanvas.AbsoluteSize.Y
-(d.ViewportSize.Y-m.UIElements.Dropdown.AbsolutePosition.Y)
+10
end
m.UIElements.MenuCanvas.Position=
UDim2.fromOffset(m.UIElements.Dropdown.AbsolutePosition.X-1,m.UIElements.Dropdown.AbsolutePosition.Y-5-n)
end
...
m.UIElements.Dropdown.Text=(p==""and"--"or p)
end


for n,o in next,m.Values do
local p={
Name=o,
Selected=false,
UIElements={},
}
p.UIElements.TabItem=f("TextButton",{
Size=UDim2.new(1,0,0,0),
AutomaticSize="Y",
BackgroundTransparency=1,
Parent=m.UIElements.Menu.CanvasGroup.ScrollingFrame,
Text=o,
TextXAlignment="Left",
...
f("UICorner",{
CornerRadius=UDim.new(0,h.MenuCorner-3)
})
})


if m.Multi then
p.Selected=table.find(m.Value,p.Name)
else
p.Selected=m.Value==p.Name
end

if p.Selected then
p.UIElements.TabItem.BackgroundTransparency=.93
end
...
m:Display()
task.spawn(function()
pcall(m.Callback,m.Value)
end)
end

p.UIElements.TabItem.MouseButton1Click:Connect(function()
if m.Multi then
if not p.Selected then
p.Selected=true
g(p.UIElements.TabItem,0.1,{BackgroundTransparency=.93}):Play()
table.insert(m.Value,p.Name)
else
if not m.AllowNone and#m.Value==1 then
return
end
p.Selected=false
g(p.UIElements.TabItem,0.1,{BackgroundTransparency=1}):Play()
for q,r in ipairs(m.Value)do
if r==p.Name then
table.remove(m.Value,q)
break
end
end
end
else
for q,r in next,m.Tabs do
...
function m.Open(p)
m.Opened=true
m.UIElements.MenuCanvas.Visible=true
m.UIElements.MenuCanvas.Active=true
m.UIElements.Menu.Size=UDim2.new(
1,0,
0,0
)
g(m.UIElements.Menu.CanvasGroup,0.1,{
GroupTransparency=0,
}):Play()
g(m.UIElements.Menu.UIScale,0.1,{
Scale=1,
}):Play()
g(m.UIElements.Menu,0.1,{
Size=UDim2.new(
1,0,
1,0
)
...
task.wait(.1)
m.UIElements.MenuCanvas.Visible=false
m.UIElements.MenuCanvas.Active=false
end

m.UIElements.Dropdown.MouseButton1Click:Connect(function()
m:Open()
end)
...
then
local q,r=m.UIElements.MenuCanvas.AbsolutePosition,m.UIElements.MenuCanvas.AbsoluteSize
if
c.X<q.X
or c.X>q.X+r.X
or c.Y<(q.Y-20-1)
...
m.UIElements.Dropdown:GetPropertyChangedSignal"AbsolutePosition":Connect(UpdatePosition)
...
function g.Input(l,m)
local n,o=a.load'm':New{
Title=m.Title,
Desc=m.Desc,
Value=m.Value,
PlaceholderText=m.PlaceholderText,
ClearTextOnFocus=m.ClearTextOnFocus,
Callback=m.Callback,
Theme=e.Window.Theme,
Parent=g.UIElements.ContainerFrame,
}
...
function g.Section(l,m)
local n,o=a.load'o':New{
Title=m.Title,
Parent=g.UIElements.ContainerFrame,
}
g.Elements[n]=o
...
for i,l in next,e.Tabs do
d(l.UIElements.Main.TextLabel,0.15,{TextTransparency=0.4}):Play()
if l.Icon and b.Icon(l.Icon)then
d(l.UIElements.Main.ImageLabel,0.15,{ImageTransparency=0.4}):Play()
end
l.Selected=false
end
...
task.spawn(function()
for m,n in next,e.Containers do
...
h.UIElements.SideBar=d("ScrollingFrame",{
...
function h.Open(u)
e(h.UIElements.Main.Background,0.25,{BackgroundTransparency=h.Transparent and 0.15 or 0},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
e(h.UIElements.Main.Main,0.25,{GroupTransparency=0},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
...
e(p,0.25,{ImageTransparency=.7},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
e(l,0.25,{Transparency=.8},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
end
function h.Close(u)
local v={}

e(h.UIElements.Main.Background,0.25,{BackgroundTransparency=1},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
e(h.UIElements.Main.Main,0.25,{GroupTransparency=1},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
e(h.UIElements.Main.UIScale,0.25,{Scale=.9},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
...
e(l,0.25,{Transparency=1},Enum.EasingStyle.Exponential,Enum.EasingDirection.Out):Play()
...
local function startResizing(u)
isResizing=true
n.Active=true
initialSize=h.UIElements.Main.Size
initialInputPosition=u.Position
e(n,0.2,{BackgroundTransparency=.65}):Play()
...
b.InputChanged:Connect(function(u)
if isResizing then
if u.UserInputType==Enum.UserInputType.MouseMovement or u.UserInputType==Enum.UserInputType.Touch then
local v=u.Position-initialInputPosition
local w=UDim2.new(0,initialSize.X.Offset+v.X*2,0,initialSize.Y.Offset+v.Y*2)

e(h.UIElements.Main,0.06,{
Size=UDim2.new(
0,math.clamp(w.X.Offset,400,700),
0,math.clamp(w.Y.Offset,350,650)
)}):Play()
end
end
end)
...
local g=protectgui or(syn and syn.protect_gui)or function()end
...
p.UIElements.Main.Gradient.UIGradient.Transparency=NumberSequence.new{
NumberSequenceKeypoint.new(0,1),
NumberSequenceKeypoint.new(1,r and 0.85 or 0.7),
}
end

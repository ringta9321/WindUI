...
local l=Instance.new(i)


for m,n in next,g.DefaultProperties[i]or{}do
l[m]=n
end

for o,p in next,j or{}do
if o~="ThemeTag"then
l[o]=p
end
end

for q,r in next,k or{}do
r.Parent=l
end

ApplyCustomProps(l,j)
return l
end

function g.Tween(i,j,k,...)
return e:Create(i,TweenInfo.new(j,...),k)
end
...
o.X.Scale,o.X.Offset+q.X,
o.Y.Scale,o.Y.Offset+q.Y
)
end
...
d.InputChanged:Connect(function(p)
if p==k and j then
update(p)
end
end)
...
c("UICorner",{
CornerRadius=UDim.new(0,e.UICorner-4),
}),
c("UIPadding",{
...
local j
local k
local l=h.Value.Default or h.Value.Min or 0

local o=l
local p=(l-(h.Value.Min or 0))/((h.Value.Max or 100)-(h.Value.Min or 0))
...
l=math.floor((h.Value.Min+s*(h.Value.Max-h.Value.Min))/h.Step+0.5)*h.Step

if l~=o then
h.UIElements.SliderIcon.Frame.Size=UDim2.new(s,0,1,0)
h.UIElements.SliderContainer.TextLabel.Text=l
o=l
task.spawn(h.Callback,l)
end
end)
...
if p.KeyCode.Name==l or l=="MouseLeft"and p.UserInputType==Enum.UserInputType.MouseButton1 or l=="MouseRight"and p.UserInputType==Enum.UserInputType.MouseButton2 then
i.Picking=false

i.UIElements.Keybind.Text=l
i.Value=l
...
local l=-35
if d.ViewportSize.Y-k.UIElements.Dropdown.AbsolutePosition.Y+l<k.UIElements.MenuCanvas.AbsoluteSize.Y+10 then
l=k.UIElements.MenuCanvas.AbsoluteSize.Y
-(d.ViewportSize.Y-k.UIElements.Dropdown.AbsolutePosition.Y)
+10
end
k.UIElements.MenuCanvas.Position=UDim2.fromOffset(k.UIElements.Dropdown.AbsolutePosition.X-1,k.UIElements.Dropdown.AbsolutePosition.Y-l)
end

function k.Display(l)
local o=k.Values
local p=""
...
c("UICorner",{
CornerRadius=UDim.new(0,s.UICorner-3),
}),
c("UIPadding",{
...
g.Elements.Paragraph=l
return Content
end
task.wait()
function g.Button(j,k)
local l,o=a.load'i':New{
Title=k.Title,
Desc=k.Desc,
Callback=k.Callback,
Theme=e.Window.Theme,
...
task.spawn(function()
for k,l in next,e.Containers do
l.AnchorPoint=Vector2.new(0,0.05)
l.Visible=false
end
...
PaddingBottom=UDim.new(0,i.UIPadding+4),
}),
e("UIListLayout",{
SortOrder="LayoutOrder",
Padding=UDim.new(0,8)
...
local p=e("Frame",{
Size=UDim2.new(1,0,1,0),
BackgroundTransparency=i.Transparent and 0.15 or 0,
ZIndex=3,
Name="Gradient"
},{
e("UIGradient",{
...
local v=u.UIStroke.UIGradient

c.RenderStepped:Connect(function(w)
v.Rotation=(v.Rotation+1)%360
end)

u.MouseEnter:Connect(function()
f(u.UIScale,.1,{Scale=.95}):Play()
end)
u.MouseLeave:Connect(function()
f(u.UIScale,.1,{Scale=1}):Play()
end)
end
...
v.Image=d.Icon(i.Icon)
u.Icon.Image=d.Icon(i.Icon)
end
...
writefile("WindUI"..i.Folder.."/Assets/Icon.png",w)
end
v.Image=getcustomasset("WindUI"..i.Folder.."/Assets/Icon.png")
u.Icon.Image=getcustomasset("WindUI"..i.Folder.."/Assets/Icon.png")
...
if not t then
u.Icon.Image=d.Icon"external-link"
end
end
end)
...
local v=t and"Press "..i.Name.." to open the Window"or"Click the Button to open the Window"

if not g then
g=not g
h.WindUI:Notify{
Title="Minimize",
Content="You've closed the Window. "..v,
Duration=5,
}
end
end)
...
b.InputBegan:Connect(function(v,w)
if w then return end

if v.KeyCode==i.ToggleKey then
if i.Closed then
i:Open()
else
i:Close()
...
local v=a.load'r'.Init(i)
function i.Tab(w,x)
task.wait()
return v.New{Title=x.Title,Icon=x.Icon,Parent=i.UIElements.SideBar}
end

local w=a.load'o'.Init(i)
function i.Dialog(x,y)
local z={
Title=y.Title or"Dialog",
Content=y.Content,
Buttons=y.Buttons or{},
}
local A=w.Create()


A.UIElements.UIListLayout=e("UIListLayout",{
...
A.UIElements.Title=e("TextLabel",{
Text=z.Title,
TextSize=20,
...
for C,D in next,z.Buttons do
local E=e("TextButton",{
Text=D.Title or"Button",
TextSize=15,
FontFace=Font.new(d.Font,Enum.FontWeight.Medium),
ThemeTag={
TextColor3="Text",
...
BackgroundTransparency=.9,
Parent=B,
Size=UDim2.new(1/#z.Buttons,-(((#z.Buttons-1)*8)/#z.Buttons),0,0),
AutomaticSize="Y",
},{
e("UICorner",{
CornerRadius=UDim.new(0,A.UICorner-6),
}),
e("UIPadding",{
PaddingTop=UDim.new(0,A.UIPadding/1.75),
PaddingLeft=UDim.new(0,A.UIPadding/1.75),
PaddingRight=UDim.new(0,A.UIPadding/1.75),
PaddingBottom=UDim.new(0,A.UIPadding/1.75),
}),
e("Frame",{
Size=UDim2.new(1,(A.UIPadding/1.75)*2,1,(A.UIPadding/1.75)*2),
Position=UDim2.new(0.5,0,0.5,0),
AnchorPoint=Vector2.new(0.5,0.5),
ThemeTag={
BackgroundColor3="Text"
...
e("UICorner",{
CornerRadius=UDim.new(0,A.UICorner-3),
}),
})
})

E.MouseEnter:Connect(function()
f(E.Frame,0.1,{BackgroundTransparency=.9}):Play()
end)
E.MouseLeave:Connect(function()
f(E.Frame,0.1,{BackgroundTransparency=1}):Play()
end)
...
l.InputBegan:Connect(function(x)
if x.UserInputType==Enum.UserInputType.MouseButton1 or x.UserInputType==Enum.UserInputType.Touch then
startResizing(x)
end
end)

b.InputChanged:Connect(function(x)
if isResizing then
if x.UserInputType==Enum.UserInputType.MouseMovement or x.UserInputType==Enum.UserInputType.Touch then
local y=x.Position-initialInputPosition
local z=UDim2.new(0,initialSize.X.Offset+y.X*2,0,initialSize.Y.Offset+y.Y*2)

f(i.UIElements.Main,0.06,{
Size=UDim2.new(
0,math.clamp(z.X.Offset,400,700),
0,math.clamp(z.Y.Offset,350,520)
)}):Play()
end
end
end)
...
if l.KeySystem.SaveKey and l.Folder then
f("TextLabel",{
Text="Your key will be saved in Workspace/"..l.Folder.."/"..game.Players.LocalPlayer.Name..".key",
TextSize=13,
FontFace=Font.new(e.Font,Enum.FontWeight.SemiBold),
TextXAlignment="Left",
TextWrapped=false,
...
if C and C==l.KeySystem.Key then
s:Close()
g(t,.1,{ImageTransparency=1}):Play()
p=true
if l.KeySystem.SaveKey and l.Folder then
writefile(l.Folder.."/"..game.Players.LocalPlayer.Name..".key",tostring(C))
end
task.spawn(function()
task.wait(0.1)
t:Destroy()
...
if l.KeySystem then
if l.KeySystem.SaveKey then
if isfile(l.Folder.."/"..game.Players.LocalPlayer.Name..".key")and l.KeySystem.Key==readfile(l.Folder.."/"..game.Players.LocalPlayer.Name..".key")then
p=true
else
KeySystem()
end
...
Callback=function(g)
game.Workspace.Camera.FieldOfView=g
end
}
...
Callback=function(g)
game.Workspace[game.Players.LocalPlayer.Name].Humanoid.WalkSpeed=g
end
}
...
Content="Selected: "..game:GetService"HttpService":JSONEncode(h),
Duration=2,
}
end
}
...
if isfile(k)then
local l=readfile(k)
return h:JSONDecode(l)
end
end

local function ListFiles()
local j={}
for k,l in ipairs(listfiles(i))do
local o=l:match"([^/]+)%.json$"
if o then
table.insert(j,o)
end
end
...
local j={}
for k,l in pairs(c:GetThemes())do
table.insert(j,k)
end
...
o:Select(c:GetCurrentTheme())

local p=f:Toggle{
Title="Toggle Window Transparency",
Callback=function(p)
c:ToggleTransparency(p)
end,
...
Callback=function()
if q~=""then
SaveFile(q,{Transparent=c:GetTransparency(),Theme=c:GetCurrentTheme()})
end
end
}
...
c:ToggleTransparency(t.Transparent)
p:SetValue(t.Transparent)
end
if t.Theme then c:SetTheme(t.Theme)end
end
end
end
}
